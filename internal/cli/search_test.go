...
// newest first, per_page ignored, before: filtering by UTC day.
func cappedServer(t *testing.T, posts []*mm.Post) *httptest.Server {
	t.Helper()
...
func TestSearchPastServerCap(t *testing.T) {
	cases := []struct {
		...
		{"all across days", makePosts(250, 7), []string{"--all"}, 250, ""},
		{"limit cuts off", makePosts(250, 7), []string{"--limit", "120"}, 120, "--limit 120"},
		{"exact limit", makePosts(250, 7), []string{"--limit", "250"}, 250, ""},
		{"default limit", makePosts(250, 7), nil, 50, "--limit 50"},
		{"one day over cap", makePosts(150, 150), []string{"--all"}, 100, "more than 100 results on 2026-09-20"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			srv := cappedServer(t, tc.posts)
